...
- **WatchlistViewModel** (`@MainActor ObservableObject`): 화면 상태(loading/normal/ws-disconnected/token-refreshing/empty)의 단일 소유자. QuoteService의 `AsyncStream`을 구독해 행 데이터를 갱신하고, 등록/삭제 사용자 이벤트를 Service·Store로 위임. 뷰는 이 ViewModel의 `@Published` 상태만 렌더한다.
